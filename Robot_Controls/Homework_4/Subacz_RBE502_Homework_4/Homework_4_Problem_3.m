%% Subacz, RBE502, Homework 4, Problem 3
function []= Problem3()
...
%% Functions
    ...
    function [dx] = plannarArmODE(t,x)
        ...
        Gmatd = [(m_1*r_1 + m_2*r_2)*g*sin(theta_d(1)) + m_2*r_2*g*sin(theta_d(1) + theta_d(2));m_2*r_2*g*sin(theta_d(1) + theta_d(2))];
        
        invM = inv(Mmat);
        invMC = invM*Cmat;
        invMG = invM*Gmat;
        tau = PDPlusFeedforwardControl(theta_d, dtheta_d, ddtheta_d, theta, dtheta); %
        torque =[torque, tau];
        dx=zeros(4,1);
        dx(1) = x(3);
        dx(2) = x(4);
