%% Subacz, RBE502, Homework 6, Problem 1D
...
function []=Figure_11_17()
    ...
    Error = Start_Pos; % Initial error position
    ...
    [T,X] = ode45(@(t,x)ODE45Controller(t,x),[0 Tf],x0);
        ...
        function [dx] = ODE45Controller(t,x)
            ...
            dx = [ dx_1 + k_1*(x_1 - x(1));
                   dy_2 + k_2*(y_2 - x(2));
                   (dy_2 + k_2*(y_2 - x(2))*cos(x(3)) - dx_1 + k_1*(x_1 - x(1))*sin(x(3)))/b];
        end
    
        for i=1:size(T)
            %Calculating the desired trajectory
            [x_1,y_2,~,~] = SquareTrajectory(T(i));
            End_Pos(i,:)=[x_1, y_2, 0]; % desired trajectory
        end
