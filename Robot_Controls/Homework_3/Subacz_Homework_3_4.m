...
function [] = PD_set_control_Law(x_pos,y_pos)
    ...
    function dx = planarArmODE(t,x,theta_1,theta_2)
        ...
        dx(1) = x(3); %dtheta1
        dx(2) = x(4); %dtheta2
        dx(3:4) = -invMC* x(3:4) - invMG + invM*tau; % because ddot theta = -M^{-1}(C \dot Theta) + M^{-1} tau
    end
...
% 
% The biggest difference between the results form part j and m is the
% inclusion of the gravity term. Comparing the Input_PD control graphs 
% between the resulting inputs converge in the linearized model from part j.
% The model from part M does not converge at in the Input_PD control graph. 
% There is always a approximate delta of -2.5 below the 0 error point.
% The same result is seem in the Link_2 Final Position graphs between 
% the two. 
%
